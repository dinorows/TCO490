...
try {
    ...
    #~dk
    $tempcreds = Get-STSSessionToken -AccessKey AKIA2WVOR3G5Z3Z5NSLB -SecretKey 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 -DurationSeconds 900

    # Get assigned EIP addresses
    $assignedEIP = Get-EC2Address -AccessKey $tempcreds.AccessKeyId -SecretKey $tempcreds.SecretAccessKey -SessionToken $tempcreds.SessionToken -Region $region | ? { $PSItem.PublicIp -in $allowedEIPs -and $PSItem.PrivateIpAddress -eq $privateIP }
    ...
    if ($assignedEIP) {
        ...
        do {
            ...
            $availableEIPs = Get-EC2Address -AccessKey $tempcreds.AccessKeyId -SecretKey $tempcreds.SecretAccessKey -SessionToken $tempcreds.SessionToken -Region $region | ? { $PSItem.PublicIp -in $allowedEIPs -and $PSItem.PrivateIpAddress -eq $null }
            if($availableEIPs.Count -gt 0) {
                ...
                try {
                    ...
                    $associationID = Register-EC2Address -AccessKey $tempcreds.AccessKeyId -SecretKey $tempcreds.SecretAccessKey -SessionToken $tempcreds.SessionToken -Region $region -AllocationId $randomAvailableEIP.AllocationId -InstanceId $instanceID
                    $associated = $true
        ...
        do {
            try {
                ...
                $associatedEIP = Get-EC2Address -AccessKey $tempcreds.AccessKeyId -SecretKey $tempcreds.SecretAccessKey -SessionToken $tempcreds.SessionToken -Region $region -AllocationId $randomAvailableEIP.AllocationId
            }
            catch {
